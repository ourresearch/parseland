...
class GrobidParser(Parser):
    ...
    def parse(self):
        form = ProcessForm(
            ...
            input_=File(file_name="contents.pdf", payload=BytesIO(self.pdf_contents),
        ...
        for tag in author_tags:
            author = {'name': None, 'affiliations': [], 'is_corresponding': tag.get('role', '') == 'corresp'}
            ...
            elif universal_aff_tag := tag.select_one('note[type=raw_affiliation]'):
        ...
        ref_tags = soup.select('div[type=references] listbibl biblstruct note[type=raw_reference]')
