...
class DergiPark(RepositoryParser):
    ...
    def parse(self):
        ...
        for author_paragraph in self.soup.findAll("p", {"id": re.compile(r'^author\d+$')}):
            [s.extract() for s in author_paragraph.find_all('small')]
            [a.extract() for a in author_paragraph.find_all('a', {'class': re.compile(r'claim-authorship')})]
                ...
                if author_strings:
                    ...
                    for author_string in author_strings:
                        if re.search(orcid_pattern, author_string):
                            author['orcid'] = re.sub(rf'.*({orcid_pattern}).*', r'\1', author_string).upper()
                        else:
